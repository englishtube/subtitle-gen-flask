...
def download_video(video_url, video_folder_path):
    yt = YouTube(video_url)

    output = yt.streams.filter(file_extension="mp4").get_highest_resolution().download(video_folder_path)
...
def format_timestamp(seconds: float, always_include_hours: bool = False):
    ...
    hours_marker = hours if always_include_hours or hours > 0 else ""
    return f"{hours_marker:02d}:{minutes:02d}:{seconds:02d}.{milliseconds:03d}"
