from flask import Flask, request, render_template, flash, send_from_directory, url_for, redirect
import os
import whisper
import shutil
import subtitle
...
def subtitle_gen():
  ...
  if(request.method == 'POST'):
    try: 
      ...
      print("Data : ",data)
      flash("enerror: English Subtitle is not generated!!", "error")
      return redirect('/')
...
def chainese():
  try:
    ...
    data = {"status":"failed","error":str(e)}
    print("Data : ", data)
    flash("cherror: Chinese Subtitle is not generated !!", "error")
    return redirect('/')
...
def japanese():
  try:
    ...
    data = {"status":"failed","error":str(e)}
    print("Data : ", data)
    flash("japerror: Japanese Subtitle is not generated !!", "error")
    return redirect('/')
...
def korean():
  try:
    ...
    data = {"status":"failed","error":str(e)}
    print("Data : ", data)
    flash("koerror: Korean Subtitle is not generated !!", "error")
    return redirect('/')
...
#       video_url = request.form['video_url']
#       downloaded_filename = os.path.join(app.config['VIDEO_UPLOAD_FOLDER'], "downloaded_video.mp4")
#       download_videos = subtitle.download_video(video_url, downloaded_filename)
...
#       audios = subtitle.get_audio(download_videos,extracted_folder_path)
#       english_srt_file = subtitle.get_subtitles(
#         audios, srt_only, output_dir, lambda audio_path: model.transcribe(audio_path, fp16=False)
#       )
